...
- 2026-06-05 — Fixed UsbStorageManager.writeFile (hub): partial writes from network/USB failures left truncated files on USB with the correct filename; future syncs found the name in getExistingFileNames and permanently skipped re-downloading → broken files stuck forever. Fix: wrap stream.copyTo in try-catch and delete the SAF file if the copy fails. Also added zero-byte check: if an existing file is 0 bytes (previous failed write), delete it and re-download instead of skipping. Both changes are in the hub APK.
- 2026-06-05 — Fixed LocalImageProcessor.fixOwnedFilesWithNullDate: was using stampFileDate (IS_PENDING trick + file rewrite) on owned WebP files; Samsung rescanned them as JPEG, found no JPEG EXIF date, and reset DATE_TAKEN to today. Also risked 0-byte files if write failed mid-truncate. Fix: use tryUpdateDateTaken (direct ContentValues update) for owned files — app owns them so no IS_PENDING needed, and no file rewrite prevents Samsung rescan loop and truncation risk.
- 2026-06-10 — Fixed infinite re-download loop. (1) Auto hub-restore removed from `ClientForegroundService.runOnHubConnect`: it re-downloaded files the phone had intentionally slimmed (esp. videos VideoSpaceManager posterised → mp4 deleted, poster kept), which got re-posterised → re-restored forever and made the hub re-back-up the re-inserted files. Full restore is now strictly user-initiated. (2) New menu item "Restore from hub" → `MainActivity.restoreFromHubNow()` calls `HubRestoreManager.restoreAll`. (3) `HubRestoreManager` now skips hub videos whose poster image is already on the phone (via `buildExistingImageStems`) so a manual restore can't re-form the loop. (4) Hub: new one-time `UsbStorageManager.removeDuplicateFiles` dedup pass (DEDUP_VERSION=1) removes same-name files duplicated across date folders (left by reorg passes), keeping the largest copy. Files changed: ClientForegroundService.kt, MainActivity.kt, menu_client.xml, HubRestoreManager.kt, UsbStorageManager.kt, HubForegroundService.kt.
- 2026-06-10 — Duplicate cleanup hardening. Manually removed the accumulated `(N)` duplicates created by the old auto-restore loop: phone (451 redundant images deleted, keeping the smallest/compressed copy per family → 1803 unique, 0 dup families) and hub USB (637 redundant media + 40 stray `photosync_state/manifest (N).json` files deleted, keeping the largest original per family → 2086 files, 0 dup families). Code: (1) hub `UsbStorageManager.removeDuplicateFiles` now groups by *family key* (strips trailing ` (N)` suffix groups + extension) and keeps the largest original (DEDUP_VERSION→2 so it re-runs). (2) New `deleteNamedVariants` helper purges all `(N)` variants before re-writing `photosync_state.json`/`photosync_manifest_*.json` — fixes the SAF auto-rename bug that was spawning `photosync_state (1..32).json` every sync.
- 2026-06-07 — Fixed wrong EXIF dates on hub USB: old compression loop created `foo (1).jpg` files with dateTaken=0; stampExifDate fell back to dateAdded (= compression date in April 2026) stamping sync date into EXIF instead of capture date. Fix: (1) stampExifDate now uses filename-parsed YYYYMMDD_HHMMSS date as highest priority, never falls back to dateAdded; (2) one-time repairExifFromFilenames pass runs on hub startup and corrects all files where EXIF date differs from filename date by >1h (fixed 870 files); (3) dateChecked cache cleared after repair so FileSyncer re-runs date-correction pass and pushes correct dates back to phone. Also fixed pre-existing compile error: TAG_FLASH_PIX_VERSION removed from ExifInterface in newer AndroidX. Hub v542 deployed.
