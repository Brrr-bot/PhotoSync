...
- 2026-06-04 — Quality uplift: VideoTranscoder bitrate 2→4 Mbps for noticeably sharper transcoded videos. VideoSpaceManager: wired up dormant COMPRESS_VERSION check (was dead code), bumped 2→3 to re-transcode existing 2-Mbps videos at the new bitrate; fixed "mark regardless" bug so failed transcodes retry instead of being permanently skipped. WebP quality already at 85 — no change needed.
- 2026-06-09 — Updated client app launcher icon: replaced placeholder "C" glyph with circular sync-arrows (Material Design sync style, 2.5× scaled) containing a photo/landscape placeholder in the centre opening. White-on-black design matches new icon set. PR #3 open on claude/session-8nz1fr.
