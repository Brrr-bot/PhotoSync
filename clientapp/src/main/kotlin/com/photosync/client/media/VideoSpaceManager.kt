...
class VideoSpaceManager(private val context: Context) {
        ...
        videos.forEachIndexed { index, v ->
            try {
                ...
                if (ageMs > OLD_AGE_MS) {
                    ...
                    val takenMs = parseDateFromName(v.name).takeIf { it > 0 } ?: v.takenMs
                    val stampedPoster = VideoThumbnailer.stampPosterExif(poster, takenMs)
                    insertPoster(v, stampedPoster, posterName) ?: run { skipped++; return@forEachIndexed }
     ...
     */
    private fun insertPoster(v: VideoRow, jpeg: ByteArray, posterName: String): Uri? {
